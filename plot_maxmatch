...
def plot_similarity_vs_acc(index_func, std_model, model_list, save_path=None):
    ...
    std_feature = model_feature.get_feature(8) # Get the feature after last block
    for model in model_list:
        model_feature.load_state_dict(model.state_dict())
        output = model_feature(testimage)
        feature = model_feature.get_feature(8)  #get_feature
        sim_list.append(index_func(std_feature, feature,0.5).item())#try 0.1,0.2,...,0.5
    # Ploting
...
if __name__ == "__main__":
    ...
    os.chdir("..")
    plot_similarity_vs_acc(similarity.maxmatch, std_model, model_list, save_path="fig1.PNG")
